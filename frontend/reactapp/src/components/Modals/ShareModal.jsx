...
import { X, Copy, Check, Link, FileCode, Users, CodeXml } from "lucide-react";
      ...
      <div className="bg-[#0B0F17] border border-gray-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100">
        ...
        <div className="flex border-b border-gray-800">
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === "edit" ? "text-blue-400 border-b-2 border-blue-400 bg-blue-500/5" : "text-gray-400 hover:text-gray-200"
              }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === "snippet" ? "text-green-400 border-b-2 border-green-400 bg-green-500/5" : "text-gray-400 hover:text-gray-200"
              }`}
          >
        ...
        <div className="p-6">
          <div className="mb-6">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              {activeTab === "edit"
                ? "Anyone with this link can join the session and edit the query in real time. They must be logged in to SQLTogether."
                : "Anyone with this link can view a copy of your query in the SQL Playground. They cannot edit your original project."
              }
            </p>
          </div>
          ...
          {!generatedLink ? (
            ...
            <div className="animate-scaleIn flex flex-col gap-4">
              <div>
                {activeTab === "snippet" && <label className="text-xs text-gray-400 mb-1 block">Link</label>}
                <div className="flex items-center gap-2 p-1.5 bg-gray-800 rounded-lg border border-gray-700">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-md transition-all ${copiedItem === 'link' ? "bg-green-500/20 text-green-400" : "bg-gray-700 hover:bg-gray-600 text-white"
                      }`}
                  >
              ...
              {activeTab === "snippet" && (
                <div>
                  <label className="text-xs text-gray-400 mb-1 block">Embed HTML</label>
                  <div className="flex items-center gap-2 p-1.5 bg-gray-800 rounded-lg border border-gray-700">
                    <input
                      readOnly
                      value={`<iframe src="${generatedLink.replace('/snippet/', '/embed/')}" width="100%" height="450px" frameborder="0"></iframe>`}
                      className="flex-1 bg-transparent text-sm text-gray-300 px-2 focus:outline-none font-mono truncate"
                    />
                    <button
                    ...
                    >
                      {copiedItem === 'embed' ? <Check className="h-4 w-4" /> : <CodeXml className="h-4 w-4" />}
                    </button>
                  </div>
                </div>
              )}
